...
func run() error {
	fmt.Println("=== BN254 G1 MSM Metal Smoke ===")
	fmt.Println()
	...
	fmt.Println()
	fmt.Println("PASS: BN254 G1 MSM Metal smoke succeeded")
	return nil
}
...
func gpuJacToGnark(in bn254gpu.G1Jac) gnarkbn254.G1Jac {
	return gnarkbn254.G1Jac{
		X: gnarkfp.Element(metal.JoinWords8(in.X)),
		Y: gnarkfp.Element(metal.JoinWords8(in.Y)),
		Z: gnarkfp.Element(metal.JoinWords8(in.Z)),
	}
}
