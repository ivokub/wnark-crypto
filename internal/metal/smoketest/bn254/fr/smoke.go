...
func run() error {
	fmt.Println("=== BN254 fr Ops Metal Smoke ===")
	fmt.Println()
	...
	fmt.Println()
	fmt.Println("PASS: BN254 fr Ops Metal smoke succeeded")
	return nil
}

func verifyStaticOps(kernel *bn254gpu.FrKernel, cases []elementCase, convertCases []convertCase) error {
	aBatch := make([]metal.U32x8, len(cases))
	bBatch := make([]metal.U32x8, len(cases))
...
func verifyBatch(kernel *bn254gpu.FrKernel, name string, op bn254gpu.FrOp, aBatch, bBatch []metal.U32x8, expected []string) error {
	fmt.Printf("4. %s... ", name)
...
func loadVectors() (frOpsVectors, error) {
	...
	path := filepath.Join(filepath.Dir(file), "..", "..", "..", "..", "testdata", "vectors", "fr", "bn254_fr_ops.json")
...
func mustU32x8(raw string) metal.U32x8 {
	var out metal.U32x8
...
func limbsToHex(in metal.U32x8) string {
	data := make([]byte, 32)
	for i, limb := range in {
