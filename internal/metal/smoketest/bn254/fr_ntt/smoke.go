...
func mustStageTwiddles(raw [][]string) [][]metal.U32x8 {
	out := make([][]metal.U32x8, len(raw))
	for i := range raw {
...
func mustBatch(raw []string) []metal.U32x8 {
	out := make([]metal.U32x8, len(raw))
	for i := range raw {
...
func verifyBatch(got []metal.U32x8, want []string) error {
	if len(got) != len(want) {
