...
import (
	...
	bn254gpu "github.com/ivokub/wnark-crypto/internal/metal/bn254"
)
...
func run() error {
	...
	aBatch := make([]metal.U32x8, size)
	bBatch := make([]metal.U32x8, size)
	regularBatch := make([]metal.U32x8, size)
	wantAdd := make([]metal.U32x8, size)
	wantSub := make([]metal.U32x8, size)
	wantMul := make([]metal.U32x8, size)
	wantMont := make([]metal.U32x8, size)
	wantRegular := make([]metal.U32x8, size)
	for i, tc := range cases {
		aBatch[i] = metal.SplitWords4([4]uint64(tc.aMont))
		bBatch[i] = metal.SplitWords4([4]uint64(tc.bMont))
		...
		wantAdd[i] = metal.SplitWords4([4]uint64(add))

		var sub gnarkfr.Element
		sub.Sub(&tc.aMont, &tc.bMont)
		wantSub[i] = metal.SplitWords4([4]uint64(sub))

		var mul gnarkfr.Element
		mul.Mul(&tc.aMont, &tc.bMont)
		wantMul[i] = metal.SplitWords4([4]uint64(mul))
	}

	if err := verifyUnary("copy", func() ([]metal.U32x8, error) { return frKernel.Copy(aBatch) }, aBatch); err != nil {
		return err
	}
	if err := verifyBinary("add", func() ([]metal.U32x8, error) { return frKernel.Add(aBatch, bBatch) }, wantAdd); err != nil {
		return err
	}
	if err := verifyBinary("sub", func() ([]metal.U32x8, error) { return frKernel.Sub(aBatch, bBatch) }, wantSub); err != nil {
		return err
	}
	if err := verifyBinary("mul", func() ([]metal.U32x8, error) { return frKernel.Mul(aBatch, bBatch) }, wantMul); err != nil {
		return err
	}
	if err := verifyUnary("to_mont", func() ([]metal.U32x8, error) { return frKernel.ToMont(regularBatch) }, wantMont); err != nil {
		return err
	}
	if err := verifyUnary("from_mont", func() ([]metal.U32x8, error) { return frKernel.FromMont(aBatch) }, wantRegular); err != nil {
		return err
	}
	if err := verifyBinary("mul_factors", func() ([]metal.U32x8, error) { return vectorKernel.MulFactors(aBatch, bBatch) }, wantMul); err != nil {
		return err
	}

	wantBitReverse := make([]metal.U32x8, size)
	logCount := bits.Len(uint(size)) - 1
	for i := 0; i < size; i++ {
		j := int(bits.Reverse64(uint64(i)) >> (64 - logCount))
		wantBitReverse[i] = aBatch[j]
	}
	if err := verifyUnary("bit_reverse_copy", func() ([]metal.U32x8, error) { return vectorKernel.BitReverseCopy(aBatch) }, wantBitReverse); err != nil {
		return err
	}
...
func verifyUnary(name string, fn func() ([]metal.U32x8, error), want []metal.U32x8) error {
	fmt.Printf("4. %s... ", name)
	got, err := fn()
	if err != nil {
...
func verifyBinary(name string, fn func() ([]metal.U32x8, error), want []metal.U32x8) error {
	return verifyUnary(name, fn, want)
}

func verifyBatch(got, want []metal.U32x8) error {
	if len(got) != len(want) {
...
func regularBigIntToU32x8(v *big.Int) metal.U32x8 {
	var out metal.U32x8
