...
import (
	...
	bls12_381gpu "github.com/ivokub/wnark-crypto/internal/metal/bls12_381"
)
...
func verifyStaticOps(kernel *bls12_381gpu.FpKernel, cases []elementCase, convertCases []convertCase) error {
	aBatch := make([]metal.U32x12, len(cases))
	bBatch := make([]metal.U32x12, len(cases))
...
func verifyBatch(kernel *bls12_381gpu.FpKernel, name string, op bls12_381gpu.FpOp, aBatch, bBatch []metal.U32x12, expected []string) error {
	fmt.Printf("4. %s... ", name)
...
func mustU32x12(hexLE string) metal.U32x12 {
...
func limbsToHex(in metal.U32x12) string {
	data := make([]byte, 48)
	for i, limb := range in {
