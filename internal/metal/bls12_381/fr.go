...
func ReadFrArithShader() (string, error) {
	return metal.ReadShader("curves/bls12_381/fr_arith.wgsl")
}

func ZeroBatch(count int) []metal.U32x8 {
	return make([]metal.U32x8, count)
}
...
func RunFrOperation(device *wgpu.Device, op FrOp, a, b []metal.U32x8) ([]metal.U32x8, error) {
...
func (k *FrKernel) Run(op FrOp, a, b []metal.U32x8) ([]metal.U32x8, error) {
...
func (k *FrKernel) Copy(a []metal.U32x8) ([]metal.U32x8, error) {
	return k.Run(FrOpCopy, a, ZeroBatch(len(a)))
}

func (k *FrKernel) Add(a, b []metal.U32x8) ([]metal.U32x8, error) {
	return k.Run(FrOpAdd, a, b)
}

func (k *FrKernel) Sub(a, b []metal.U32x8) ([]metal.U32x8, error) {
	return k.Run(FrOpSub, a, b)
}

func (k *FrKernel) Mul(a, b []metal.U32x8) ([]metal.U32x8, error) {
	return k.Run(FrOpMul, a, b)
}

func (k *FrKernel) ToMont(a []metal.U32x8) ([]metal.U32x8, error) {
	return k.Run(FrOpToMont, a, ZeroBatch(len(a)))
}

func (k *FrKernel) FromMont(a []metal.U32x8) ([]metal.U32x8, error) {
	return k.Run(FrOpFromMont, a, ZeroBatch(len(a)))
}

func inferCount(a, b []metal.U32x8) (int, error) {
	switch {
...
func normalizeBatch(in []metal.U32x8, count int) []metal.U32x8 {
	if len(in) == count {
		return in
	}
	out := make([]metal.U32x8, count)
	copy(out, in)
	return out
}

func flattenBatch(in []metal.U32x8) []byte {
...
func unflattenBatch(data []byte, count int) []metal.U32x8 {
	out := make([]metal.U32x8, count)
	for i := 0; i < count; i++ {
		base := i * frElementBytes
		for j := range out[i] {
