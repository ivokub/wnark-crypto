...
func ReadFpArithShader() (string, error) {
	return metal.ReadShader("curves/bls12_381/fp_arith.wgsl")
}

func ZeroFpBatch(count int) []metal.U32x12 {
	return make([]metal.U32x12, count)
}
...
func RunFpOperation(device *wgpu.Device, op FpOp, a, b []metal.U32x12) ([]metal.U32x12, error) {
...
func (k *FpKernel) Run(op FpOp, a, b []metal.U32x12) ([]metal.U32x12, error) {
...
func inferFpCount(a, b []metal.U32x12) (int, error) {
	switch {
...
func normalizeFpBatch(in []metal.U32x12, count int) []metal.U32x12 {
	if len(in) == count {
		return in
	}
	out := make([]metal.U32x12, count)
	copy(out, in)
	return out
}

func flattenFpBatch(in []metal.U32x12) []byte {
...
func unflattenFpBatch(data []byte, count int) []metal.U32x12 {
	out := make([]metal.U32x12, count)
	for i := 0; i < count; i++ {
		base := i * fpElementBytes
		for j := range out[i] {
