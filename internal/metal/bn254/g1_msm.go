...
func (k *G1MSMKernel) RunAffineNaive(bases []G1Affine, scalars []metal.U32x8, termsPerInstance int) ([]G1Jac, error) {
	out, _, err := k.RunAffineNaiveProfiled(bases, scalars, termsPerInstance)
	return out, err
}

func (k *G1MSMKernel) RunAffineNaiveProfiled(bases []G1Affine, scalars []metal.U32x8, termsPerInstance int) ([]G1Jac, G1MSMProfile, error) {
...
func (k *G1MSMKernel) RunAffinePippenger(bases []G1Affine, scalars []metal.U32x8, termsPerInstance int, window uint32) ([]G1Jac, error) {
...
func (k *G1MSMKernel) RunAffinePippengerProfiled(bases []G1Affine, scalars []metal.U32x8, termsPerInstance int, window uint32) ([]G1Jac, G1MSMProfile, error) {
	if termsPerInstance <= 0 {
		return nil, G1MSMProfile{}, fmt.Errorf("termsPerInstance must be positive")
	}
	if len(bases) != len(scalars) {
...
func extractWindowDigit(scalar metal.U32x8, bitOffset, window uint32) uint32 {
	if window == 0 {
		return 0
	}
	word := bitOffset / 32
