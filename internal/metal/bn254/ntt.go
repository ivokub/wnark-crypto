...
func ReadFrNTTShader() (string, error) {
	return metal.ReadShader("curves/bn254/fr_ntt.wgsl")
}
...
func (k *FrNTTKernel) RunStage(input, twiddles []metal.U32x8, m int) ([]metal.U32x8, error) {
...
func (k *FrNTTKernel) ForwardDIT(input []metal.U32x8, stageTwiddles [][]metal.U32x8) ([]metal.U32x8, error) {
	out, _, err := k.ForwardDITProfiled(input, stageTwiddles)
	return out, err
}

func (k *FrNTTKernel) ForwardDITProfiled(input []metal.U32x8, stageTwiddles [][]metal.U32x8) ([]metal.U32x8, FrNTTProfile, error) {
	totalStart := time.Now()
	if len(input) == 0 {
...
func (k *FrNTTKernel) InverseDIT(input []metal.U32x8, inverseStageTwiddles [][]metal.U32x8, scale metal.U32x8) ([]metal.U32x8, error) {
	out, _, err := k.InverseDITProfiled(input, inverseStageTwiddles, scale)
	return out, err
}

func (k *FrNTTKernel) InverseDITProfiled(input []metal.U32x8, inverseStageTwiddles [][]metal.U32x8, scale metal.U32x8) ([]metal.U32x8, FrNTTProfile, error) {
	totalStart := time.Now()
	if len(input) == 0 {
		return nil, FrNTTProfile{}, nil
	}
...
func (k *FrNTTKernel) ForwardDITFullPathProfiled(input []metal.U32x8, stageTwiddles [][]metal.U32x8) ([]metal.U32x8, FrNTTProfile, error) {
	...
	return k.fullPathTransform(input, stageTwiddles, nil, metal.U32x8{}, false)
}
...
func (k *FrNTTKernel) fullPathTransform(input []metal.U32x8, stageTwiddles [][]metal.U32x8, aux []metal.U32x8, scale metal.U32x8, applyScale bool) ([]metal.U32x8, FrNTTProfile, error) {
	count := len(input)
	totalStart := time.Now()
	dataSize := uint64(count * frElementBytes)
	...
	if applyScale {
		scaleBatch := make([]metal.U32x8, count)
		for i := range scaleBatch {
			scaleBatch[i] = scale
		}
