...
import (
	...
	"github.com/ivokub/wnark-crypto/internal/metal/benchutil"
	bls12381 "github.com/ivokub/wnark-crypto/internal/metal/bls12_381"
	bn254 "github.com/ivokub/wnark-crypto/internal/metal/bn254"
)
	...
	return benchutil.RunVectorBenchmarks(iters, []benchutil.VectorBenchCase[bn254.FrVectorProfile]{
		{
			...
			GPU: func() ([]metal.U32x8, bn254.FrVectorProfile, error) {
	...
	return benchutil.RunVectorBenchmarks(iters, []benchutil.VectorBenchCase[bls12381.FrVectorProfile]{
		{
			...
			GPU: func() ([]metal.U32x8, bls12381.FrVectorProfile, error) {
	...
	return benchutil.RunNTTBenchmarks(iters, []benchutil.NTTBenchCase[bn254.FrNTTProfile]{
		{
			...
			GPU: func() ([]metal.U32x8, bn254.FrNTTProfile, error) {
	...
	return benchutil.RunNTTBenchmarks(iters, []benchutil.NTTBenchCase[bls12381.FrNTTProfile]{
		{
			...
			GPU: func() ([]metal.U32x8, bls12381.FrNTTProfile, error) {
				out, profile, err := forwardKernel.kernel.ForwardDITFullPathProfiled(inputGPU, stageTwiddles)
				if err != nil {
...
func buildBN254Twiddles(domain *gnarkbnfft.Domain, size int) ([][]metal.U32x8, [][]metal.U32x8, metal.U32x8, error) {
	twiddles, err := domain.Twiddles()
	if err != nil {
		return nil, nil, metal.U32x8{}, err
	}
	...
	logN := len(twiddles)
	stageTwiddles := make([][]metal.U32x8, logN)
	inverseStageTwiddles := make([][]metal.U32x8, logN)
	...
	return stageTwiddles, inverseStageTwiddles, metal.SplitWords4([4]uint64(domain.CardinalityInv)), nil
}

func buildBLSTwiddles(domain *gnarkblsfft.Domain) ([][]metal.U32x8, [][]metal.U32x8, metal.U32x8, error) {
	twiddles, err := domain.Twiddles()
	if err != nil {
		return nil, nil, metal.U32x8{}, err
	}
...
func bn254VectorToGPU(in gnarkbnfr.Vector) []metal.U32x8 {
	out := make([]metal.U32x8, len(in))
	for i := range in {
		out[i] = metal.SplitWords4([4]uint64(in[i]))
	}
	return out
}

func blsVectorToGPU(in gnarkblsfr.Vector) []metal.U32x8 {
	out := make([]metal.U32x8, len(in))
	for i := range in {
		out[i] = metal.SplitWords4([4]uint64(in[i]))
	}
...
func bn254PointsToGPU(in []gnarkbn254.G1Affine) []bn254.G1Affine {
	...
	for i := range in {
		out[i] = bn254.G1Affine{
			X: metal.SplitWords4([4]uint64(in[i].X)),
			Y: metal.SplitWords4([4]uint64(in[i].Y)),
		}
	}
	return out
}

func bn254ScalarsToGPU(in []gnarkbnfr.Element) []metal.U32x8 {
	out := make([]metal.U32x8, len(in))
	for i := range in {
...
func bn254ScalarToRegularWords(v gnarkbnfr.Element) metal.U32x8 {
	bytesBE := v.Bytes()
	var out metal.U32x8
	for i := 0; i < 8; i++ {
...
func equalBN254GPUToCPU(gpu bn254.G1Jac, want gnarkbn254.G1Affine) bool {
	gotJac := gnarkbn254.G1Jac{
		X: gnarkbnfp.Element(metal.JoinWords8(gpu.X)),
		Y: gnarkbnfp.Element(metal.JoinWords8(gpu.Y)),
		Z: gnarkbnfp.Element(metal.JoinWords8(gpu.Z)),
	}
