...
func runBenchmarks(
	...
	cpuForwardMs := timeCPU(iters, func() { _ = cpuForward(domain, input) })
	coldForward, warmForward, forwardOut, err := timeNTTProfiled(iters, func() ([]curvegpu.U32x8, bls12381.FrNTTProfile, error) {
	...
	cpuInverseMs := timeCPU(iters, func() { _ = cpuInverse(domain, forwardExpected) })
	coldInverse, warmInverse, inverseOut, err := timeNTTProfiled(iters, func() ([]curvegpu.U32x8, bls12381.FrNTTProfile, error) {
...
func timeNTTProfiled(iters int, fn func() ([]curvegpu.U32x8, bls12381.FrNTTProfile, error)) (bls12381.FrNTTProfile, bls12381.FrNTTProfile, []curvegpu.U32x8, error) {
	last, cold, err := fn()
	if err != nil {
		return bls12381.FrNTTProfile{}, bls12381.FrNTTProfile{}, nil, err
	}
	if iters == 1 {
		return cold, cold, last, nil
	}
	var warm bls12381.FrNTTProfile
	for i := 0; i < iters; i++ {
		var profile bls12381.FrNTTProfile
		last, profile, err = fn()
		if err != nil {
			return bls12381.FrNTTProfile{}, bls12381.FrNTTProfile{}, nil, err
		}
