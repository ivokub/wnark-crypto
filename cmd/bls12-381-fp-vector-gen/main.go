...
func main() {
	outPath := flag.String("out", filepath.FromSlash("testdata/vectors/fp/bls12_381_phase3_ops.json"), "write JSON output to path")
	flag.Parse()

	rng := rand.New(rand.NewSource(20260405))

	vectors := phase3Vectors{
		...
		NormalizeCases: []normalizeCase{
			...
			{Name: "q_minus_one", InputBytesLE: regularHex(regularQMinusOne()), ExpectedBytesLE: regularHex(regularQMinusOne())},
			{Name: "q", InputBytesLE: regularHex(regularModulus()), ExpectedBytesLE: regularHex(regularUint64(0))},
			{Name: "q_plus_one", InputBytesLE: regularHex(addBig(regularModulus(), regularUint64(1))), ExpectedBytesLE: regularHex(regularUint64(1))},
			{Name: "two_q_minus_one", InputBytesLE: regularHex(subBig(mulBig(regularModulus(), regularUint64(2)), regularUint64(1))), ExpectedBytesLE: regularHex(regularQMinusOne())},
		},
...
func buildElementCase(name string, aRegular, bRegular *big.Int) elementCase {
	...
	var add, sub, negA, dblA, mul, sqA gnarkfp.Element
	add.Add(&aMont, &bMont)
	sub.Sub(&aMont, &bMont)
	negA.Neg(&aMont)
	dblA.Double(&aMont)
...
func toMont(regular *big.Int) gnarkfp.Element {
	var z gnarkfp.Element
	z.SetBigInt(regular)
	return z
}

func zeroMont() gnarkfp.Element {
	var z gnarkfp.Element
	z.SetZero()
	return z
}

func montHex(z gnarkfp.Element) string {
	data := make([]byte, 48)
	for i := 0; i < 6; i++ {
...
func regularHex(v *big.Int) string {
	bytes := v.FillBytes(make([]byte, 48))
...
func regularModulus() *big.Int {
	return new(big.Int).Set(gnarkfp.Modulus())
}

func regularQMinusOne() *big.Int {
	return new(big.Int).Sub(gnarkfp.Modulus(), big.NewInt(1))
}
...
func regularQMinus(delta uint64) *big.Int {
	return new(big.Int).Sub(gnarkfp.Modulus(), new(big.Int).SetUint64(delta))
}
...
func regularCeilHalfModulus() *big.Int {
	return new(big.Int).Sub(gnarkfp.Modulus(), regularFloorHalfModulus())
}
...
func randomFieldBigInt(rng *rand.Rand) *big.Int {
	return new(big.Int).Rand(rng, gnarkfp.Modulus())
}
