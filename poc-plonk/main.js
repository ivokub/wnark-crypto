...
function fixtureBasePath(config) {
  return `/poc-plonk/fixtures/${config.curve}/2pow${config.sizeLog}/commit${config.commitments}`;
}
...
async function runSelected() {
  ...
  appendLog(`commitments = ${config.commitments}`);
  appendLog(`prove_runs = ${config.proveRuns}`);
  appendLog("note = Proof bytes are not compared because PLONK proving is randomized.");
  appendLog("");
  ...
  try {
    ...
    setStatus("PASS");
    appendLog("");
    appendLog("PASS: PLONK TS browser POC completed");
  } catch (error) {
    setStatus("FAIL");
    appendLog("");
