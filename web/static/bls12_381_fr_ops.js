...
async function fetchVectors() {
  const text = await fetchText("/testdata/vectors/fr/bls12_381_phase2_ops.json?v=2");
...
fn fr_ops_main(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  if (i >= params.count) { return; }
  let base = i * 8u;
  output[base + 0u] = 0xfffffffeu;
  output[base + 1u] = 0x00000001u;
  output[base + 2u] = 0x00034802u;
  output[base + 3u] = 0x5884b7fau;
  output[base + 4u] = 0xecbc4ff5u;
  output[base + 5u] = 0x998c4fefu;
  output[base + 6u] = 0xacc5056fu;
  output[base + 7u] = 0x1824b159u;
}`;
  }
...
function createKernel(device, shaderCode, limbCount, opcode) {
  ...
  const shaderModule = device.createShaderModule({
    label: "bls12-381-fr-shader",
    code: specializedShader,
  });
  const bindGroupLayout = device.createBindGroupLayout({
    label: "bls12-381-fr-bgl",
    entries: [
  ...
  const pipelineLayout = device.createPipelineLayout({
    label: "bls12-381-fr-pl",
    bindGroupLayouts: [bindGroupLayout],
  });
  const pipeline = device.createComputePipeline({
    label: "bls12-381-fr-pipeline",
...
async function runOp(device, kernel, opcode, inputAHex, inputBHex) {
  ...
  const inputABuffer = createStorageBuffer(device, "bls12-381-fr-input-a", paddedBytes, GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST);
  const inputBBuffer = createStorageBuffer(device, "bls12-381-fr-input-b", paddedBytes, GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST);
  const outputBuffer = createStorageBuffer(device, "bls12-381-fr-output", paddedBytes, GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC);
  const readbackBuffer = createStorageBuffer(device, "bls12-381-fr-readback", paddedBytes, GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ);
  const uniformBuffer = createStorageBuffer(device, "bls12-381-fr-params", FR_UNIFORM_BYTES, GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST);
  ...
  const encoder = device.createCommandEncoder({ label: "bls12-381-fr-encoder" });
...
async function runBrowserSmoke() {
  const lines = ["=== BLS12-381 fr Phase 2 Browser Smoke ===", ""];
  writeLog(lines);
  ...
  const [shaderCode, vectors] = await Promise.all([
    fetchText("/shaders/curves/bls12_381/fr_arith.wgsl?v=2"),
    fetchVectors(),
  ]);
  lines[lines.length - 1] += " OK";
  writeLog(lines);
  ...
  lines.push("");
  lines.push("PASS: BLS12-381 fr browser smoke succeeded");
  writeLog(lines);
  return lines;
}
...
if (params.get("autorun") === "1") {
  ...
  writeLog([
    "=== BLS12-381 fr Phase 2 Browser Smoke ===",
    "",
    "Press Run to execute the BLS12-381 fr smoke test in browser WebGPU.",
  ]);
}
