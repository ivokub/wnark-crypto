...
function buildSparseSignedBucketMetadata(scalarWords, count, termsPerInstance, window) {
...
function scalarToKernelPoint(scalarHexLE) {
  return {
...
function makeRandomScalarHexLE(seed) {
  const out = new Uint8Array(32);
  let state = seed >>> 0;
  for (let i = 0; i < out.length; i += 1) {
    state ^= state << 13;
...
async function runOpProfiled(device, kernel, opcode, inputA, inputB, extraParams) {
...
function createPointStorageBuffer(device, label, points, minCount = points.length) {
...
function createEmptyPointStorageBuffer(device, label, count) {
...
function createU32StorageBuffer(device, label, values) {
...
function createParamsBuffer(device, label, values) {
  const buffer = device.createBuffer({
    label,
    size: UNIFORM_BYTES,
...
function createBindGroupForBuffers(device, kernel, label, inputA, inputB, output, params, meta0, meta1, meta2) {
...
async function submitKernelProfiled(device, kernel, bindGroup, count, label) {
  const start = performance.now();
...
async function readbackPointBufferProfiled(device, buffer, count) {
...
async function buildBases(device, kernel, generator, oneMontZ, count) {
...
async function runMSMProfiled(device, kernel, bases, scalars) {
  const totalStart = performance.now();
  let uploadMs = 0;
  let kernelMs = 0;
  let readbackMs = 0;
...
async function reduceAffineBucketsProfiled(device, kernel, bucketLists) {
  const profile = {
    partitionMs: 0,
    uploadMs: 0,
    kernelMs: 0,
...
async function reduceWindowsProfiled(device, kernel, bucketSums, count, numWindows, bucketCount) {
  const profile = {
    partitionMs: 0,
    uploadMs: 0,
    kernelMs: 0,
...
async function combineWindowsProfiled(device, kernel, windowSums, count, numWindows, window) {
  const profile = {
    partitionMs: 0,
    uploadMs: 0,
    kernelMs: 0,
...
async function runPippengerMSMProfiled(device, kernel, bases, scalars, window) {
  const totalStart = performance.now();
  const count = 1;
  ...
  const bucketKernelMs = await submitKernelProfiled(device, kernel.bucket, bucketBindGroup, bucketCountOut, "g1-pip-bucket");
  ...
  const windowKernelMs = await submitKernelProfiled(device, kernel.windowSparse, windowBindGroup, count * metadata.numWindows * 64, "g1-pip-window-sparse");
  ...
  const finalKernelMs = await submitKernelProfiled(device, kernel.combine, finalBindGroup, count, "g1-pip-final");
...
async function benchmarkMSM(device, kernel, bases, scalars, iters, run) {
...
async function runBenchmark() {
  ...
  try {
    ...
    const [shaderText, phase7] = await Promise.all([
      ...
      fetchJSON("/testdata/vectors/g1/bn254_phase7_scalar_mul.json"),
    ]);
