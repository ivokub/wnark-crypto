...
function getConfig(): G2OpsConfig {
...
function expectPointBatch(name: string, got: readonly CurveGPUG2JacobianPoint[], want: readonly JacobianPoint[], lines: string[]): void {
...
function expectAffineBatch(name: string, got: readonly CurveGPUG2AffinePoint[], want: readonly AffinePoint[], lines: string[]): void {
  if (got.length !== want.length) {
    throw new Error(`${name}: length mismatch got=${got.length} want=${want.length}`);
  }
  for (let i = 0; i < got.length; i += 1) {
...
async function runSmoke(config: G2OpsConfig): Promise<void> {
  ...
  try {
    ...
    lines.push(`cases.g2 = ${vectors.point_cases.length}`);
    ...
    const oneFp2 = { c0_bytes_le: bytesToHex(oneMont), c1_bytes_le: bytesToHex(curve.fp.zero()) };
    const zero = zeroFp2(curve.g2.componentBytes);
    const jacInfinityWant = vectors.point_cases.map(() => ({ x: oneFp2, y: oneFp2, z: zero }));

    expectPointBatch("copy", await g2.copyBatch(pJacobian), vectors.point_cases.map((item) => item.p_jacobian), lines);
    writeLog(lines);
    expectPointBatch("jac_infinity", await g2.jacobianInfinityBatch(vectors.point_cases.length), jacInfinityWant, lines);
    writeLog(lines);
    expectPointBatch("affine_to_jac", await g2.affineToJacobianBatch(pAffine), vectors.point_cases.map((item) => item.p_jacobian), lines);
    writeLog(lines);
    expectPointBatch("neg_jac", await g2.negJacobianBatch(pJacobian), negWant, lines);
    writeLog(lines);
    expectAffineBatch("jac_to_affine", await g2.jacobianToAffineBatch(pJacobian), affineWant, lines);
    writeLog(lines);
    expectPointBatch("double_jac", await g2.doubleJacobianBatch(pJacobian), doubleWant, lines);
    writeLog(lines);
    expectPointBatch("add_mixed", await g2.addMixedBatch(pJacobian, qAffine), addWant, lines);
    writeLog(lines);
    expectPointBatch("affine_add", await g2.affineAddBatch(pAffine, qAffine), affineAddWant, lines);
    writeLog(lines);

    lines.push("");
    lines.push(`PASS: ${curveDisplayName(config.curve)} G2 browser smoke succeeded`);
    writeLog(lines);
    setStatus("Pass");
    setPageState("pass");
  } catch (error) {
...
if (params.get("autorun") === "1") {
  ...
  writeLog([`=== ${config.title} ===`, "", `Press Run to execute the ${config.curve} G2 smoke test in browser WebGPU.`]);
}
