...
async function runOp(
  ...
  const inputABuffer = createStorageBuffer(device, "bls12-381-fr-input-a", inputBytes, GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST);
  const inputBBuffer = createStorageBuffer(device, "bls12-381-fr-input-b", inputBytes, GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST);
  const outputBuffer = createStorageBuffer(device, "bls12-381-fr-output", inputBytes, GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC);
  const readbackBuffer = createStorageBuffer(device, "bls12-381-fr-readback", inputBytes, GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ);
...
async function runBrowserSmoke(): Promise<string[]> {
  const lines: string[] = ["=== BLS12-381 fr Phase 2 Browser Smoke ===", ""];
  writeLog(lines);
...
async function main(): Promise<void> {
  ...
  try {
    ...
    writeLog(["=== BLS12-381 fr Phase 2 Browser Smoke ===", "", `FAIL: ${message}`]);
    setPageState("fail");
    setStatus("Fail");
    throw error;
  } finally {
