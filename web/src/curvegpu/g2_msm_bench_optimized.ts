...
import { createBindGroupForBuffers, createEmptyPointStorageBuffer, createMSMKernelSet, createParamsBuffer, createStorageBufferFromBytes, createU32StorageBuffer, submitKernel, type Kernel } from "./msm_gpu_runtime.js";
...
function shaderPartsForCurve(curve: SupportedCurveID): readonly string[] {
  if (curve === "bn254") {
    return [
      ...
      "/shaders/common/g2_msm.wgsl?v=1",
    ];
  }
  return [
...
function createWeightedG2PippengerRuntime(
  device: GPUDevice,
  shaderCode: string,
  labelPrefix: string,
): PippengerRuntime {
