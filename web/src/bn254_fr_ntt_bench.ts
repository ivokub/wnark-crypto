...
function buildRegularStageWords(domain: DomainMetadata, inverse: boolean): Uint32Array[] {
...
async function prepareDomainMont(device: GPUDevice, arithKernel: Kernel, domain: DomainMetadata, count: number): Promise<{
...
function createStageResources(device: GPUDevice, count: number, stageWordBatches: Uint32Array[]): StageResource[] {
  const stageResources: StageResource[] = [];
  for (const words of stageWordBatches) {
    ...
    stageResources.push({ twiddleBuffer, uniformBuffer });
  }
  return stageResources;
}

function destroyStageResources(stageResources: StageResource[]): void {
...
async function runBenchmark(): Promise<void> {
  ...
  try {
    ...
    const [arithShader, vectorShader, nttShader, domainMeta] = await Promise.all([
      ...
      fetchJSON<DomainMetadataFile>("/testdata/vectors/fr/bn254_ntt_domains.json"),
    ]);
