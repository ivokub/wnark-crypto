...
func createPipeline(device *wgpu.Device, bufs *bufferSet) (*pipelineSet, error) {
	...
	bindGroup, err := device.CreateBindGroup(&wgpu.BindGroupDescriptor{
		...
		Entries: []wgpu.BindGroupEntry{
			...
			{Binding: 2, Buffer: bufs.uniform, Size: uniformBufSize},
		},
	})
	if err != nil {
	...
	pipeline, err := device.CreateComputePipeline(&wgpu.ComputePipelineDescriptor{
		Label: "sum-pipeline", Layout: plLayout, Module: shader, EntryPoint: "cs_main",
	})
	if err != nil {
...
func dispatchAndReadBack(device *wgpu.Device, ps *pipelineSet, bufs *bufferSet) (uint32, error) {
	...
	if err := device.WaitIdle(); err != nil {
		return 0, fmt.Errorf("wait idle: %w", err)
	}
	fmt.Println("OK")
